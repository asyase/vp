<?php
	...
	function listAllNewsToEdit(){
		...
		while($stmt->fetch()){
			...
			$newshtml .= "\t" .'<p><a href="editnews.php?news=' .$idfromdb .'">Toimeta seda uudist</a>' ."\n";
			$newshtml .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
			$newshtml .= "\t" .'<a href="listnews.php?deletenews=' .$idfromdb .'">Kustuta seda uudist</a></p>' ."\n";
			$newshtml .= "</div> \n";
		}
		if(empty($newshtml)){
	...
	function readNewsToEdit($id){
		$news = [];
		$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
		$stmt = $conn->prepare("SELECT vpnews_id, title, content, expire, deleted, filename, alttext, vpnewsphotos.vpnewsphotos_id AS vpnewsphotos_id FROM vpnews LEFT JOIN vpnewsphotos on vpnewsphotos.vpnewsphotos_id = vpnews.photoid WHERE vpnews_id= ? GROUP BY vpnews.vpnews_id ORDER BY vpnews_id");
		echo $conn->error;
		$stmt->bind_param("i", $id);
		$stmt->bind_result($id, $titlefromdb, $contentfromdb, $expirefromdb, $deletedfromdb, $filenamefromdb, $alttextfromdb, $vpnewsphotos_id);
		$stmt->execute();
		if($stmt->fetch()){
			$news["id"] = $id;
