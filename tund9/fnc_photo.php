...
// ---------------------------------------------------------------------------------------------------------------------------------------------------------
// PRIVATE PILTIDE LUGEMINE
	// function countPrivatePhotos($privacy){
		// $photocount = 0;
		// $conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
		// $stmt = $conn->prepare("SELECT COUNT(vpphotos_id) FROM vpphotos WHERE privacy = 1 AND deleted IS NULL");
		// echo $conn->error;
		// $stmt->bind_result($photocountfromdb);
		// $stmt->execute();
		// if($stmt->fetch()){
...
// ---------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// function readAllPrivatePhotoThumbs($privacy){
		// $thumbshtml = "<p>Kahjuks fotosid ei leitud!</p> \n";
		// $conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
		// $stmt = $conn->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy = 1 AND deleted IS NULL ORDER BY vpphotos_id DESC");
		// echo $conn->error;
		// $stmt->bind_result($filenamefromdb, $alttextfromdb);
		// $stmt->execute();
		// $temphtml = null;
